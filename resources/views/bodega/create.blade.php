    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('bodega.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-text-input id="categoria" name="categoria" type="text" class="mt-1 block w-full" value="{{ old('categoria') }}" required />
                        <x-input-error class="mt-2" :messages="$errors->get('categoria')" />
                    </div>

                    <div>
                        <x-input-label for="unidad" :value="__('Unidad')" />
                        <x-text-input id="unidad" name="unidad" type="text" class="mt-1 block w-full" value="{{ old('unidad') }}" required />
                        <x-input-error class="mt-2" :messages="$errors->get('unidad')" />
                    </div>

                    <div>
                        <x-input-label for="stock_inicial" :value="__('Stock inicial')" />
                        <x-text-input id="stock_inicial" name="stock_inicial" type="number" min="0" step="1" inputmode="numeric" pattern="[0-9]*" class="mt-1 block w-full" value="{{ old('stock_inicial', 0) }}" required />
                        <x-input-error class="mt-2" :messages="$errors->get('stock_inicial')" />
                    </div>

                    <div>
                        <x-input-label for="stock_minimo" :value="__('Stock mínimo')" />
                        <x-text-input id="stock_minimo" name="stock_minimo" type="number" min="0" step="1" inputmode="numeric" pattern="[0-9]*" class="mt-1 block w-full" value="{{ old('stock_minimo', 0) }}" required />
                        <x-input-error class="mt-2" :messages="$errors->get('stock_minimo')" />
                    </div>

                    <div>
                        <x-input-label for="ubicacion" :value="__('Ubicación')" />
                        <x-text-input id="ubicacion" name="ubicacion" type="text" class="mt-1 block w-full" value="{{ old('ubicacion') }}" />
                    ...
                    <div>
                        ...
                        <select id="estado" name="estado" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            ...
                            <option value="inactivo" @selected(old('estado', 'activo') === 'inactivo')>inactivo</option>
                        </select>
