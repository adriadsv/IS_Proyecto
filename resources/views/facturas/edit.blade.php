<x-app-layout>
    ...
    @php
        ...
        foreach ($detalle as $item) {
            ...
            $defaultProductos[] = $item['producto_codigo'] ?? '';
            $defaultCantidades[] = $item['cantidad'] ?? '';
            $defaultPrecios[] = $item['precio'] ?? '';
        }

        $oldProductos = old('detalle_producto_codigo', $defaultProductos);
        $oldCantidades = old('detalle_cantidad', $defaultCantidades);
        $oldPrecios = old('detalle_precio', $defaultPrecios);
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('facturas.update', $factura['numero']) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="cliente" name="cliente" type="text" class="mt-1 block w-full" value="{{ old('cliente', $factura['cliente'] ?? '') }}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('cliente')" />
                        </div>

                        <div>
                            <x-input-label for="fecha" :value="__('Fecha')" />
                            <x-text-input id="fecha" name="fecha" type="date" class="mt-1 block w-full" value="{{ old('fecha', $factura['fecha'] ?? '') }}" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="tipo_numero_comprobante" name="tipo_numero_comprobante" type="text" class="mt-1 block w-full" value="{{ old('tipo_numero_comprobante', $factura['tipo_numero_comprobante'] ?? '') }}" required />
                        ...
                        <div>
                            ...
                            <select id="metodo_pago" name="metodo_pago" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="">-</option>
                                <option value="Efectivo" @selected(old('metodo_pago', $factura['metodo_pago'] ?? '') === 'Efectivo')>Efectivo</option>
                                <option value="Tarjeta" @selected(old('metodo_pago', $factura['metodo_pago'] ?? '') === 'Tarjeta')>Tarjeta</option>
                                <option value="Transferencia" @selected(old('metodo_pago', $factura['metodo_pago'] ?? '') === 'Transferencia')>Transferencia</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="descuento" name="descuento" type="number" step="0.01" class="mt-1 block w-full" value="{{ old('descuento', $factura['descuento'] ?? '') }}" />
                            <x-input-error class="mt-2" :messages="$errors->get('descuento')" />
                        </div>

                        <div>
                            <x-input-label for="observacion" :value="__('Observación')" />
                            <x-text-input id="observacion" name="observacion" type="text" class="mt-1 block w-full" value="{{ old('observacion', $factura['observacion'] ?? '') }}" />
