            ...
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('clientes.update', $cliente) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="nombres" name="nombres" type="text" class="mt-1 block w-full" value="{{ old('nombres', $cliente->nombres) }}" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="direccion" name="direccion" type="text" class="mt-1 block w-full" value="{{ old('direccion', $cliente->direccion) }}" />
                        <x-input-error class="mt-2" :messages="$errors->get('direccion')" />
                    </div>

                    <div>
                        <x-input-label for="telefono" :value="__('Teléfono')" />
                        <x-text-input id="telefono" name="telefono" type="tel" maxlength="10" inputmode="numeric" pattern="[0-9]*" class="mt-1 block w-full" value="{{ old('telefono', $cliente->telefono) }}" />
                        <x-input-error class="mt-2" :messages="$errors->get('telefono')" />
                    </div>

                    <div>
                        <x-input-label for="correo" :value="__('Correo')" />
                        <x-text-input id="correo" name="correo" type="email" class="mt-1 block w-full" value="{{ old('correo', $cliente->correo) }}" />
                    ...
                    <div>
                        ...
                        <select id="estado" name="estado" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            ...
                            <option value="inactivo" @selected(old('estado', $cliente->estado) === 'inactivo')>Eliminado</option>
                        </select>
