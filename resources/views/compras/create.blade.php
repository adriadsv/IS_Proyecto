                ...
                <form method="POST" action="{{ route('compras.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="fecha_compra" name="fecha_compra" type="date" class="mt-1 block w-full" value="{{ old('fecha_compra') }}" required />
                        ...
                        <div>
                            ...
                            <select id="tipo_comprobante" name="tipo_comprobante" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                <option value="nota_venta" @selected(old('tipo_comprobante') === 'nota_venta')>Nota de venta</option>
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="numero_comprobante" name="numero_comprobante" type="text" class="mt-1 block w-full" value="{{ old('numero_comprobante') }}" required />
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @for ($i = 0; $i < 5; $i++)
                                        <tr>
                                            <td class="px-3 py-2">
                                                <select name="detalle_producto_id[]" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                                    ...
                                                    @foreach ($productos as $prod)
                                                        <option value="{{ $prod->id }}" @selected((old('detalle_producto_id.'.$i) == $prod->id))>
                                            ...
                                            <td class="px-3 py-2">
                                                <input name="detalle_cantidad[]" type="number" min="1" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" value="{{ old('detalle_cantidad.'.$i) }}" />
                                            </td>
                                            <td class="px-3 py-2">
                                                <input name="detalle_costo_unitario[]" type="number" step="0.01" min="0.01" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" value="{{ old('detalle_costo_unitario.'.$i) }}" />
                                            </td>
                                        </tr>
                                    @endfor
                                </tbody>
