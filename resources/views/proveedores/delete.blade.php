        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg p-6 space-y-4">
                <div class="text-sm text-gray-700">
                    {{ __('¿Seguro que deseas eliminar el proveedor seleccionado?') }}
                </div>

                <div class="text-sm text-gray-900">
                    <div><span class="font-semibold">Identificación/RUC:</span> {{ $proveedor->identificacion }}</div>
                    <div><span class="font-semibold">Razón social:</span> {{ $proveedor->razon_social }}</div>
                </div>

                <form method="POST" action="{{ route('proveedores.destroy', $proveedor) }}" class="flex items-center gap-2">
                    @csrf
                    @method('DELETE')
